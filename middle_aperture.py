...
for n in nlist:
    ...
    n2=1 #number of groups of size 2
    ...
    while checkerCondicionesIniciales and counterCheckerCondicionesIniciales<101:
        ...
        for i in range(n3):
            ...
            while positionChecker:
                positionChecker=False
                a[contadorGlobal, 2]=(width-2.0)*np.random.rand()+1.0 #y coordinate
                a[contadorGlobal ,1]=(length-2.0)*np.random.rand()+1.0 #x coordinate
                for j in range(contadorGlobal):
                    mod=(a[contadorGlobal, 2]-a[j,2])**2+(a[contadorGlobal, 1]-a[j,1])**2
                    if mod<0.35**2 and counterPositionChecker<101:
                        if counterPositionChecker>99:
            ...
            a[contadorGlobal,3]=a[contadorGlobal, 0]*np.random.normal(1.3, 0.2) #Velocity in x
            ...
            a[contadorGlobal, 4]=0 #Velocity in y
            a[contadorGlobal+1, 4]=0
        ...
        for i in range(restantes):
            ...
            while positionChecker:
                positionChecker=False
                a[contadorGlobal,2]=(width-2.0)*np.random.rand()+1.0 #X position
                a[contadorGlobal, 1]=(length-2.0)*np.random.rand()+1.0 # y position
                for j in range(contadorGlobal):
                    mod=(a[contadorGlobal, 1]-a[j,1])**2+(a[contadorGlobal, 2]-a[j,2])**2
                    if mod<0.35**2 and counterPositionChecker<101:
                        if counterPositionChecker>99:
                        ...
                        counterPositionChecker+=1
            a[contadorGlobal,3]=np.random.normal(1.3, 0.2)*a[contadorGlobal, 0] #velocity in x
            a[contadorGlobal, 4]=0 #velocity in y
            a[contadorGlobal, 5]=abs(a[contadorGlobal, 3]) #desired velocity
    ...
    while t<tmax:   
        ...
        f1x=np.zeros(n, dtype=float)
        f1y=np.zeros(n, dtype=float)
    
        for i in range(n):
            if a[i,2]>=width-anchura: 
                if (a[i,0]<0 and a[i,1]>=length/2) or (a[i,0]>0 and a[i,1]<=length/2): 
                    goalx=a[i,0]
                    goaly=width-anchura-PersonalLim
                    goaly-=a[i,2]
                    modgoal=np.sqrt(goalx**2+goaly**2)
                    goalx/=modgoal
                    goaly/=modgoal
                    f1x[i]+=(goalx*a[i,5]-a[i,3])/tau
                    f1y[i]+=(goaly*a[i,5]-a[i,4])/tau
                    dist=length/2-a[i, 1]
                    f1x[i]-=aparam*np.exp(-abs(dist)/b)*np.sign(dist) 
                    posOtherx=dist
                    posOthery=anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                    f1x[i]-=moduloFuerza*posOtherx/modposOther
                    f1y[i]-=moduloFuerza*posOthery/modposOther
                elif (a[i,0]>0 and a[i,1]>length/2) or (a[i,0]<0 and a[i,1]<length/2):

                    
                    f1x[i]+=(a[i,5]*a[i,0]-a[i,3])/tau
                    f1y[i]+=(-a[i,4])/tau
                    posOtherx=length/2-a[i,1]

                    moduloFuerza=aparam*np.exp(-abs(posOtherx)/b)*np.sign(posOtherx)
                    f1x[i]+=moduloFuerza
                    posOthery=anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                ...
                if (a[i,0]<0 and a[i,1]>=length/2) or (a[i,0]>0 and a[i,1]<=length/2): 
                    ...
                    moduloFuerza=aparam*np.exp(-abs(posOtherx)/b)
                    f1x[i]-=moduloFuerza*np.sign(posOtherx)
                    posOthery=width-anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                    f1x[i]-=moduloFuerza*posOtherx/modposOther
    ...
    #Repulsive force
        ...
        f2x=np.zeros(n, dtype=float)
        f2y=np.zeros(n, dtype=float)
        ...
        for i in range(n):
            if a[i,2]>=width-anchura:
                if (a[i,0]<0 and a[i,1]>=length/2) or (a[i,0]>0 and a[i,1]<=length/2):
                    ...
                    goalx/=modgoal
                    goaly/=modgoal
                    f2x[i]+=(goalx*a[i,5]-a[i,3])/tau
                    f2y[i]+=(goaly*a[i,5]-a[i,4])/tau
                    dist=length/2-a[i, 1]
                    f2x[i]-=aparam*np.exp(-abs(dist)/b)*np.sign(dist) 
                    posOtherx=dist
                    posOthery=anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                    f2x[i]-=moduloFuerza*posOtherx/modposOther
                    f2y[i]-=moduloFuerza*posOthery/modposOther
                elif (a[i,0]>0 and a[i,1]>length/2) or (a[i,0]<0 and a[i,1]<length/2):

                    
                    f2x[i]+=(a[i,5]*a[i,0]-a[i,3])/tau
                    f2y[i]+=(-a[i,4])/tau
                    posOtherx=length/2-a[i,1]
                ...
                if (a[i,0]<0 and a[i,1]>=length/2) or (a[i,0]>0 and a[i,1]<=length/2): 
                    ...
                    posOtherx=length/2-a[i,1]
                    moduloFuerza=aparam*np.exp(-abs(posOtherx)/b)
                    f2x[i]-=moduloFuerza*np.sign(posOtherx)
                    posOthery=width-anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                    f2x[i]-=moduloFuerza*posOtherx/modposOther
    ...
    #Repulsive force
        ...
        f3x=np.zeros(n, dtype=float)
        f3y=np.zeros(n, dtype=float)
        ...
        for i in range(n):
            if a[i,2]>=width-anchura: 
                if (a[i,0]<0 and a[i,1]>=length/2) or (a[i,0]>0 and a[i,1]<=length/2):
                    ...
                    goalx/=modgoal
                    goaly/=modgoal
                    f3x[i]+=(goalx*a[i,5]-a[i,3])/tau
                    f3y[i]+=(goaly*a[i,5]-a[i,4])/tau
                    dist=length/2-a[i, 1]
                    f3x[i]-=aparam*np.exp(-abs(dist)/b)*np.sign(dist)
                    posOtherx=dist
                    posOthery=anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                    f3x[i]-=moduloFuerza*posOtherx/modposOther
                    f3y[i]-=moduloFuerza*posOthery/modposOther
                elif (a[i,0]>0 and a[i,1]>length/2) or (a[i,0]<0 and a[i,1]<length/2):
                    f3x[i]+=(a[i,5]*a[i,0]-a[i,3])/tau
                    f3y[i]+=(-a[i,4])/tau
                    posOtherx=length/2-a[i,1]
                    moduloFuerza=aparam*np.exp(-abs(posOtherx)/b)*np.sign(posOtherx)
                    f3x[i]+=moduloFuerza
                ...
                if (a[i,0]<0 and a[i,1]>=length/2) or (a[i,0]>0 and a[i,1]<=length/2): 
                    ...
                    posOtherx=length/2-a[i,1]
                    moduloFuerza=aparam*np.exp(-abs(posOtherx)/b)
                    f3x[i]-=moduloFuerza*np.sign(posOtherx)
                    posOthery=width-anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                    f3x[i]-=moduloFuerza*posOtherx/modposOther
        ...
        f4x=np.zeros(n, dtype=float)
        f4y=np.zeros(n, dtype=float)
        ...
        for i in range(n):
            if a[i,2]>=width-anchura:
                if (a[i,0]<0 and a[i,1]>=length/2) or (a[i,0]>0 and a[i,1]<=length/2):
                    ...
                    goalx/=modgoal
                    goaly/=modgoal
                    f4x[i]+=(goalx*a[i,5]-a[i,3])/tau
                    f4y[i]+=(goaly*a[i,5]-a[i,4])/tau
                    dist=length/2-a[i, 1]
                    f4x[i]-=aparam*np.exp(-abs(dist)/b)*np.sign(dist)
                    posOtherx=dist
                    posOthery=anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                    f4x[i]-=moduloFuerza*posOtherx/modposOther
                    f4y[i]-=moduloFuerza*posOthery/modposOther
                elif (a[i,0]>0 and a[i,1]>length/2) or (a[i,0]<0 and a[i,1]<length/2):
                    f4x[i]+=(a[i,5]*a[i,0]-a[i,3])/tau
                    f4y[i]+=(-a[i,4])/tau
                    posOtherx=length/2-a[i,1]
                    moduloFuerza=aparam*np.exp(-abs(posOtherx)/b)*np.sign(posOtherx)
                    f4x[i]+=moduloFuerza
                ...
                if (a[i,0]<0 and a[i,1]>=length/2) or (a[i,0]>0 and a[i,1]<=length/2): 
                    ...
                    posOtherx=length/2-a[i,1]
                    moduloFuerza=aparam*np.exp(-abs(posOtherx)/b)
                    f4x[i]-=moduloFuerza*np.sign(posOtherx)
                    posOthery=width-anchura-a[i,2]
                    modposOther=np.sqrt(posOtherx**2+posOthery**2)
                    moduloFuerza=aparam*np.exp(-modposOther/b)
                    f4x[i]-=moduloFuerza*posOtherx/modposOther
        ...
        t+=hstep
        if t>1 and CheckerCruzado:
            for i in range(n):
                if (a[i,0]>0 and a[i,1]>length/2) or (a[i,0]<0 and a[i,1]<length/2):
                    HanCruzado[i]=1
            if sum(HanCruzado)<n-10:
                CheckerCruzado=True
            else: 
                CheckerCruzado=False
                print('90 per cent of pedestrians have crossed by the time',t)
        if t>10:
        ...
        if t>10:
            ...
            ImageCounter+=1
            plt.plot([length/2, length/2], [0, anchura], color='black')
            plt.plot([length/2, length/2],[width, width-anchura], color='black')
            plt.plot([0, length], [width, width], color='black')
            plt.plot([0, length], [0,0], color='black')
